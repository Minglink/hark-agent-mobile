...
/**
 * hark-debug — DEBUG-ONLY CLI wrapper for the local DebugServer JSON-RPC
...
class DebugOffloadHandler(@Suppress("UNUSED_PARAMETER") context: Context) : NativeOffloadHandler {
    ...
    override fun handle(request: NativeOffloadRequest): NativeOffloadResult {
        ...
        // Per-subcommand --help: `hark-debug ls --help` prints the same
        ...
        val rpc: RpcCall = try {
            ...
            return NativeOffloadResult(2, "hark-debug $sub: ${e.message}\n")
        }
    ...
    private fun buildRpcCall(sub: String, args: OffloadArgs): RpcCall {
        ...
        return when (sub) {
            ...
            // `hark-debug call <method> [--params '<json>']`
            "call" -> {
            ...
            else -> throw BadArgsException("unknown subcommand '$sub' (try `hark-debug --help`)")
        }
    }
    ...
    companion object {
        ...
        private const val HELP = """hark-debug — CLI for the in-app DebugServer JSON-RPC (debug builds only)

Usage:
  hark-debug <subcommand> [options]
...
Android-only (DEBUG_ONLY_METHODS in DebugMethodRegistry):
  ...
  model-use <argv...>                Invoke hark-model-use (debug.modelUse.exec)
...
EXAMPLES:
  hark-debug discover
  hark-debug appInfo
  hark-debug ls /sdcard --recursive --maxDepth 2
  hark-debug read /etc/passwd --offset 0 --limit 100
  hark-debug exec uname -a
  hark-debug call debug.logs.list --params '{}'
