#!/usr/bin/env python3
"""hark-mcp-cli — MCP client CLI for the Hark agent (iSH / PRoot).
...
Diagnostics go to /var/hark/mcp-servers/mcp-cli.log, never to stdout.
...
published in /tmp/hark-mcp-daemon.port. info / add / remove / enable / disable
...
LOG_PATH = "/var/hark/mcp-servers/mcp-cli.log"

PID_FILE = "/tmp/hark-mcp-daemon.pid"
PORT_FILE = "/tmp/hark-mcp-daemon.port"  # daemon publishes its 127.0.0.1 port here
LOCK_FILE = "/tmp/hark-mcp-daemon.lock"  # cold-start fork guard (one winner forks)
...
def maybe_start_daemon():
    ...
    try:
        ...
        import logging
        daemon_log = "/var/hark/mcp-servers/mcp-daemon.log"
        try:
...
def cmd_add(args, pretty):
    ...
    if wants_oauth:
        out["oauth_note"] = (
            ...
            "the CLI — open Hark Settings > MCP Integrations > %s and tap "
...
USAGE = """hark-mcp-cli — MCP (Model Context Protocol) client for the Hark agent.

Usage: hark-mcp-cli <command> [args] [--pretty]
...
Commands:
  ...
  add ... --oauth-client-id <id> --oauth-auth-endpoint <url> --oauth-token-endpoint <url>
      [--oauth-client-secret <secret>] [--oauth-scopes "s1 s2"] [--oauth-redirect-uri <uri>]
                                        ...
                                        Hark Settings > MCP Integrations >
                                        <server> > Authorize.
...
Files:
  Servers:  /var/hark/mcp-servers/servers.json   (mcpServers object, Claude-Desktop compatible)
  Log:      /var/hark/mcp-servers/mcp-cli.log

Examples:
  hark-mcp-cli list --pretty
  hark-mcp-cli tools notion
  hark-mcp-cli call notion search --input '{"q":"x"}'
  hark-mcp-cli add --name notion --url https://mcp.notion.so/mcp --header "Authorization: Bearer $NOTION_TOKEN"
  hark-mcp-cli add --name github --command npx --args "-y @modelcontextprotocol/server-github" --env "GITHUB_TOKEN=$GITHUB_TOKEN"
  hark-mcp-cli add --name atlassian --command uvx --args "mcp-atlassian" --startup-timeout 120
  hark-mcp-cli add --name gworkspace --url https://my-gws-mcp.example.com/mcp \
...
Server startup timeout (Hark config, not part of the MCP protocol):
...
def main():
    ...
    try:
        if cmd == "list":
            ...
            sys.stderr.write("Run 'hark-mcp-cli --help' for usage.\n")
