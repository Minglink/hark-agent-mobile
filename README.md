...
<p align="center">
  ...
  <a href="LICENSE"><img src="https://img.shields.io/badge/License-CC%20BY--NC--SA%204.0-orange.svg?style=flat-square" alt="License: CC BY-NC-SA 4.0 (非商用)" /></a>
...
- **分布式任务拆解**：主代理可根据任务复杂度，自发生成多个轻量化子代理（Subagent）并行处理复杂任务。
- **混合专家（MoA）协作模型**：支持多智能体分工协作，如调研代理（Research Subagent）、代码审查代理、工具调用代理等。
- **专用管理与监控**：
  - 配备 `DelegateTaskTool` 工具，实现主代理与子代理间的精准任务派发与结果回收。
  - 前台内置 `SubagentStatusBar` 状态栏与 `SubagentInspectorSheet` 检查抽屉，实时查看各子代理的运行阶段、日志流与资源消耗。
  - 严格的上下文隔离机制，防止子代理执行产生的噪音污染全局会话历史。
...
### 4. 📋 自主任务规划引擎（TodoWrite Task Engine）
- **任务目标状态机**：Agent 在面对复杂或多阶段长流程任务时，会自动调用 `todo_write` 工具建立清晰的任务执行计划。
- **移动端交互进度卡片**：在聊天窗口上方以动画折叠卡片展示即时进度、完成状态（`[x]`, `[~]`, `[ ]`）与高优先级标签，任务推进一目了然。

### 5. ⚡ 智能上下文压缩（Micro-Compaction）
...
- **[Hark-1.0.0.apk](https://github.com/Minglink/hark-agent-mobile/releases/download/v1.0.0/Hark-1.0.0.apk)**（推荐下载：采用深邃黑曜石渐变黑与 Cyber Cyan 视觉重构，集成完整 arm64 PRoot Alpine Linux 沙箱、多子代理协作、自定义系统提示词与 XOR 动态防篡改校验）
...
本项目采用 **[CC BY-NC-SA 4.0（知识共享 署名-非商业性使用-相同方式共享 4.0 国际开源协议）](LICENSE)**。

- **个人学习与研究**：完全免费开源，允许自由阅读源码、学习、调试与个人非商业自用。
- **🚫 严禁未经授权的商业用途**：未经官方著作权人（Minglink）正式书面授权，严禁任何形式的商业盈利、有偿售卖、二次包装倒卖、付费套壳或集成进商业闭源软件中。
- **商业授权**：如需商业合作或商业授权，请通过官方交流 QQ 群（`338431075`）或 GitHub 取得联系。
- 第三方底层依赖及其原开源许可证信息详见 [THIRD_PARTY_LICENSES.md](THIRD_PARTY_LICENSES.md)。
