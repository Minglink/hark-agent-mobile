...
# anything with that label 鈥?chmod +x succeeds, the exec still gets EACCES.
...
# native_offload initialises, so any "does the sandbox start?" check passes 鈥?# but the first execve("/bin/sh") fails and every command in the app comes
...
# releases 鈥?the loader's .text differs between toolchain generations (the
...
fetch_talloc() {
    ...
    # talloc.c expects Samba's `replace.h` 鈥?a compat shim that pulls in the
...
build_proot() {
    ...
    # src/ 鈥?object files land next to sources, cleaned by `make clean`.
    # Note: Makefile's default CPPFLAGS adds `-D_FILE_OFFSET_BITS=64
    # -D_GNU_SOURCE -I. -I$(VPATH)` 鈥?we must preserve -I. since proot
...
install_asset() {
    ...
    # build. The binary is reproducible from source 鈥?rerun this script.
    rm -f "$OUTPUT_BIN.bak"
    ...
    # extracted Alpine rootfs) can never be exec'd. Only apk_data_file 鈥?    # i.e. what the installer extracts into nativeLibraryDir from lib/ in
    # the APK 鈥?is executable. proot's answer is these loaders: the loader
    ...
    # 鈥?which is why a "does the sandbox start?" smoke test passes 鈥?but the
    ...
    # not shared objects.
    # 鈹€鈹€ DO NOT install the fork-built loaders. 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
    #
...
verify_artifacts() {
    log_info "Verifying installed artifacts鈥?

    local failed=0

    # path:min_bytes:arch 鈥?the size floor catches a truncated or zero-length
    ...
    for entry in "${required[@]}"; do
        ...
        size=$(wc -c < "$path" | tr -d ' ')
        if [ "$size" -lt "$min" ]; then
            log_warn "TOO SMALL: $name is $size bytes (expected >= $min) 鈥?truncated install?"
            failed=1
            continue
        fi
        ...
        log_success "  鉁?$name ($size bytes)"
    done

    if [ "$failed" -ne 0 ]; then
        log_error "Artifact verification failed 鈥?the APK built from this tree would have a broken sandbox (shell commands would return '[Shell not running]'). Fix the errors above and rerun."
    fi
